...
- **Association access**: `entity.ownedAssociations` does not exist in SDK v5. Associations live on the domain model, not on individual entities. Use a **two-pass** approach: Pass 1 extracts entities and builds an `entityByName` map; Pass 2 iterates `domainModel.associations`, calls `.load()` on each, then attaches it to the owning entity via `entityByName.get(assoc.parent?.name)`. Skipping this two-pass pattern causes all entities to have empty association arrays.
- **Association type detection**: `assoc.type` is an `AssociationType` enum (extends `AbstractEnum`). Use `String(assoc.type).includes('ReferenceSet')` to detect many-to-many; anything else is one-to-many (`Reference`). Direct `===` comparison against the enum value does not work reliably across module boundaries.
...
- **CustomWidget (Marketplace)**: entity bindings use `DirectEntityRef.entityQualifiedName` and `DataSource.entityQualifiedName` (SDK v5 — not `.qualifiedName` or `.entity.qualifiedName`). These widgets (DataGrid 2, ListView, etc.) are otherwise opaque — columns and full datasource config are not accessible through the SDK. When the page's entity has reverse one-to-many relations (other entities that FK back to it), the generator produces clickable avatar cards with native `<dialog>` popups showing the related rows. When no reverse relations exist, it falls back to a `Object.keys(item)` dynamic table. The `onclick` inside each card must embed the dialog ID via EJS interpolation (`<%= item.id %>`); do not reference `item` as a plain JavaScript variable in onclick attributes — `item` only exists in EJS template scope.
