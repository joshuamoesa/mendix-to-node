...
function widgetToHtml(widget: MendixWidget, indent: string): string {
  const i = indent
  const i2 = indent + '  '
  ...
  switch (widget.kind) {
    case 'DataView': {
      ...
      const children = widget.children.map(c => widgetToHtml(c, i2)).filter(Boolean).join('\n')
      return `${i}<form method="POST" action="/${formEntity.toLowerCase()}/save">
...
${i}</div>`
    ...
    default:
      return widget.children.length > 0
        ? widget.children.map(c => widgetToHtml(c, i)).filter(Boolean).join('\n')
        : `${i}<!-- ${widget.rawType} -->`
  }
}
...
function generateEjsTemplate(page: MendixPage): string {
  ...
  const widgetHtml = page.widgets.map(w => widgetToHtml(w, '  ')).filter(Boolean).join('\n\n')

  return `<%- include('../views/layout', { title: '${title}' }) -%>
...
router.get('/${routePath}', async (req: Request, res: Response) => {
  try {
    ...
    res.status(500).render('error', { message: 'Failed to load ${page.name}' })
  }
})
...
router.post('/${routePath}/save', async (req: Request, res: Response) => {
  try {
    ...
    res.status(500).render('error', { message: 'Failed to save' })
  }
})
